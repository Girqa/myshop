...
public class GlobalExceptionHandler {
  ...
  public Mono<Rendering> handleException(Exception e, ServerWebExchange exchange) {
    ...
    return Mono.just(Rendering.view("error/common-error")
        .modelAttribute("status", HttpStatus.PAYMENT_REQUIRED)
  ...
  public Mono<Rendering> handlePaymentException(
      PaymentException ignored,
      ServerWebExchange exchange
  ) {
    ServerHttpResponse response = exchange.getResponse();
    response.setStatusCode(HttpStatus.BAD_REQUEST);

    return Mono.just(Rendering.view("error/not-enough-money")
        .modelAttribute("status", HttpStatus.PAYMENT_REQUIRED)
        .modelAttribute("timestamp", LocalDateTime.now())
        .build());
  }
